	...
	if(!cl->download)
	{
		...
		if ( !(sv_allowDownload->integer & DLF_ENABLE) ||
			...
			if(unreferenced)
			{
				...
				Com_Printf("clientDownload: %d : \"%s\" cannot download qw pk3 files\n", (int) (cl - svs.clients), cl->downloadName);
					Com_sprintf(errorMessage, sizeof(errorMessage), "Cannot autodownload qw pk3 file \"%s\"", cl->downloadName);
			}
