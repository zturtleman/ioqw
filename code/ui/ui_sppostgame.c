...
void UI_SPPostgameMenu_Cache(void) {
	...
	if (buildscript) {
		...
		trap_S_RegisterSound("snd/c/announce/youwin.wav", qfalse);
	}
}
...
void UI_SPPostgameMenu_f(void) {
	...
	if (playerGameRank != 1) {
		postgameMenuInfo.winnerSound = trap_S_RegisterSound(va("snd/c/announce/%s_wins.wav", postgameMenuInfo.placeNames[0]), qfalse);
		trap_Cmd_ExecuteText(EXEC_APPEND, "music music/loss\n");
	} else {
		postgameMenuInfo.winnerSound = trap_S_RegisterSound("snd/c/announce/youwin.wav", qfalse);
