		...
		while (s && *s && count < len) {
			...
			if (Q_IsColorString(s)) {
				...
				if (style == ITEM_TEXTSTYLE_SHADOWED || style == ITEM_TEXTSTYLE_SHADOWEDMORE) {
					...
					Text_PaintChar(x + xadj + ofs, y - yadj + ofs, glyph->imageWidth, glyph->imageHeight, useScale, glyph->s, glyph->t, glyph->s2, glyph->t2, glyph->glyph);
					trap_R_SetColor(newColor);
					colorBlack[3] = 1.0;
				}

				Text_PaintChar(x + xadj, y - yadj, glyph->imageWidth, glyph->imageHeight, useScale, glyph->s, glyph->t, glyph->s2, glyph->t2, glyph->glyph);
