...
// screen positions
#define MAPCOLUMN_LEFTX COLUMN_LEFT
#define MAPCOLUMN_RIGHTX COLUMN_RIGHT
#define MAPBUTTONS_X (SMALLCHAR_WIDTH)
#define MAPARRAYCOLUMN_X (SMALLCHAR_WIDTH * 13)
...
void CreateServer_MapPage_CopyCustomLimitsToControls(void) {
	...
	for (i = 0; i < NUMMAPS_PERPAGE; i++) {
		memcpy(s_mapcontrols.displayFragLimit[i].field.buffer, s_scriptdata.map.data[base + i].fragLimit, MAX_LIMIT_BUF);
	}

	for (i = 0; i < NUMMAPS_PERPAGE; i++) {
		memcpy(s_mapcontrols.displayTimeLimit[i].field.buffer, s_scriptdata.map.data[base + i].timeLimit, MAX_LIMIT_BUF);
	}
}
...
void CreateServer_MapPage_MenuInit(void) {
	...
	s_mapcontrols.actionDest.generic.x = MAPACTIVATE_X;
	s_mapcontrols.actionDest.generic.y = y + LINE_HEIGHT;
