...
static void FirstRun_SaveChanges(void) {
	...
	// name
	trap_Cvar_Set("name", s_firstrun.name.field.buffer);
}
...
static void FirstRun_SetMenuItems(void) {
	...
	// name
	Q_strncpyz(s_firstrun.name.field.buffer, UI_Cvar_VariableString("name"), sizeof(s_firstrun.name.field.buffer));
