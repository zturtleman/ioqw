...
Expects an index into the index_maplist[], NOT an Arena number.
=======================================================================================================================================
*/
...
static void MapSelect_ToggleMultiSelect(void) {
	...
	trap_Cvar_SetValue("ui_map_multisel", s_mapselect.multisel.curvalue);
...
static void MapSelect_LoadMaps(const char *mapname, qboolean cache) {
	int i, count, matchbits, j, nchars, lastspace, secondline, count2;
	const char *info;
	char *buf;
	...
	for (i = 0; i < s_mapselect.maxMapsPerPage; i++) {
		...
		for (j = 0; j < s_scriptdata.map.num_maps; j++) {
			...
			if (Q_stricmp(Info_ValueForKey(info, "map"), s_scriptdata.map.data[j].shortName) == 0) {
				s_mapselect.maptext_color[i] = color_red;
				break;
			}
		}
	}
...
static void MapSelect_MenuEvent(void *ptr, int event) {
	...
	switch (((menucommon_s *)ptr)->id) {
		...
		case ID_MAPSELECT_LISTVIEW:
			trap_Cvar_SetValue("ui_map_list", s_mapselect.listview.curvalue);
			MapSelect_SetViewType();
			MapSelect_SetNewMapPage();
			MapSelect_UpdateInterface();
			break;
		case ID_MAPSELECT_MAPICONS:
			trap_Cvar_SetValue("ui_mapicons", s_mapselect.mapicons.curvalue);
			break;
	}
}
...
static void MapSelect_DrawMapPic(void *self) {
	int x, y, w, h, n, i, multi, id, hasfocus, secondline, offset;
	menubitmap_s *b;
	vec4_t tempcolor;
	float *color;
	...
	if (secondline) {
		offset += LINE_HEIGHT;
		UI_DrawString(x, y - LINE_HEIGHT, &s_mapselect.mapdrawname[n][secondline], UI_CENTER|UI_SMALLFONT, color);
	}
...
static void MapSelect_ScrollListDraw(void *ptr) {
	int x, u, y, i, base, column, style, charwidth, charheight, lineheight, index, map, offset;
	float *color;
	qboolean hasfocus;
	menulist_s *l;
...
static qboolean MapSelect_ListIndexFromCursor(int *pos) {
	int x, y, w, cursorx, cursory, column, charwidth, charheight, lineheight;
	menulist_s *l;
...
static void MapSelect_DrawListMapPic(void) {
	static int oldindex = 0, maptime = 0;
	int x, y, colw, colh, index, base, delta;
	vec4_t colour;
...
static void MapSelect_MenuDraw(void) {
	...
	if (s_mapselect.nomaps) {
		UI_DrawProportionalString(320, 240 - 32, "NO MAPS FOUND", UI_CENTER, color_nomap);
		return;
	}
...
static void MapSelect_MenuInit(int gametype, int index, const char *mapname) {
	...
	s_mapselect.maptype.generic.y = 24 + 36;
...
//	s_mapselect.icona.generic.name = "menu/medals/medal_excellent";
	...
	s_mapselect.icona.generic.y = 28 + (SMALLCHAR_HEIGHT - 32) / 2;
...
//	s_mapselect.iconb.generic.name = "menu/medals/medal_victory";
	...
	s_mapselect.iconb.generic.y = 28 + (SMALLCHAR_HEIGHT - 32) / 2;
	...
	s_mapselect.filter.generic.y = 28;
	...
	s_mapselect.mapicons.generic.y = 28 + LINE_HEIGHT + 8;
	...
	s_mapselect.mapicons.curvalue = (int)Com_Clamp(0, MAPICONS_MAX - 1, ui_mapicons.integer);
	...
	s_mapselect.arrows.generic.y = 435;
	...
	s_mapselect.previous.generic.y = 435;
	...
	s_mapselect.next.generic.y = 435;
	...
	s_mapselect.cancel.generic.x = 0;
	s_mapselect.cancel.generic.y = 435;
	s_mapselect.cancel.width = BUTTON_WIDTH;
	s_mapselect.cancel.height = BUTTON_HEIGHT;
	...
	s_mapselect.accept.generic.flags = QMF_RIGHT_JUSTIFY|QMF_PULSEIFFOCUS;
	s_mapselect.accept.generic.id = ID_MAPSELECT_OK;
	s_mapselect.accept.generic.callback = MapSelect_MenuEvent;
	s_mapselect.accept.generic.x = 640;
	s_mapselect.accept.generic.y = 435;
	s_mapselect.accept.width = BUTTON_WIDTH;
	s_mapselect.accept.height = BUTTON_HEIGHT;
	...
	s_mapselect.maplist.generic.y = 84;
	...
	s_mapselect.multisel.generic.y = 495 - 3 * SMALLCHAR_HEIGHT;
	...
	s_mapselect.listview.generic.y = 495 - 2 * SMALLCHAR_HEIGHT;
