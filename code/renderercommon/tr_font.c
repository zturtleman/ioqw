...
/**************************************************************************************************************************************
 ...
 As of this writing (Nov, 2000) Team Arena uses these fonts for all of the ui and about 90% of the cgame presentation. A few areas of
 the CGAME were left uses the old fonts since the code is shared with standard Q3A.

 Q3A 1.25+ and Team Arena were shipped with the font rendering code disabled, there was a patent held by Apple at the time which
 FreeType MIGHT infringe on. This removed any potential patent issues and it kept us (id Software) from having to  distribute an actual
 TrueTrype font which is 1. expensive to do and 2. seems to require an act of god to accomplish.

 What we did was pre-render the fonts using FreeType (which is why we leave the FreeType credit in the credits) and then saved off the
 glyph data and then hand touched up the font bitmaps so they scale a bit better in GL.

 In the UI Scripting code, a scale of 1.0 is equal to a 48 point font. In Team Arena, we use three or four scales, most of them exactly
 equaling the specific rendered size. We rendered three sizes in Team Arena, 12, 16, and 20.

 To generate new font data you need to go through the following steps:

 1. delete the fontImage_x_xx.tga files and fontImage_xx.dat files from the fonts path.
 2. in an ui script, specificy a font, smallFont, and bigFont keyword with font name and point size. the original TrueType fonts must
    exist in fonts at this point.
 3. run the game, you should see things normally.
 4. exit the game and there will be three dat files and at least three tga files. The tga's are in 256x256 pages so if it takes three
    images to render a 24 point font you will end up with fontImage_0_24.tga through fontImage_2_24.tga.
 5. in future runs of the game, the system looks for these images and data files when a pecific point sized font is rendered and loads
    them for use.
 6. because of the original beta nature of the FreeType code you will probably want to hand touch the font bitmaps.

 Currently a define in the project turns on or off the FreeType code which is currently defined out. To pre-render new fonts you need
 enable the define (BUILD_FREETYPE) and uncheck the exclude from build check box in the FreeType2 area of the Renderer project.
**************************************************************************************************************************************/
...
=======================================================================================================================================
*/
void RenderSpans(FT_Library library, FT_Outline *const outline, SpanList *list) {
...
Render an outline glyph with a optional border around it. This function and the "span" rendering code is based on freetype2
example2.cpp by Erik Möller, which was released into the public domain.
=======================================================================================================================================
*/
	...
	if (borderWidth > 0) {
		...
		if (FT_Get_Glyph(glyph, &glyphTemp) == 0) {
			...
			if (glyphTemp->format == FT_GLYPH_FORMAT_OUTLINE) {
				...
				FT_Stroker_Set(stroker, (int)(borderWidth * 64), FT_STROKER_LINECAP_ROUND, FT_STROKER_LINEJOIN_ROUND, 0);
...
qboolean R_LoadDynamicFont(const char *fontName, int pointSize, float borderWidth, qboolean forceAutoHint, fontInfo_t *font) {
	...
	// point sizes are for a virtual 640 * 480 window
	...
	// make a 256x256 image buffer, once it is full, register it, clean it and keep going until all glyphs are rendered
