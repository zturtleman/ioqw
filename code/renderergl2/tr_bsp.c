...
static void ParseMesh(dsurface_t *ds, drawVert_t *verts, float *hdrVertColors, msurface_t *surf) {
	...
	// copy the level of detail origin, which is the center of the group of all curves that must subdivide the same to avoid cracking
	for (i = 0; i < 3; i++) {
		...
		for (k = 0; k < grid1->width - 2; k += 2) {
			for (m = 0; m < 2; m++) {
				...
				for (l = 0; l < grid2->width - 1; l++) {
					...
					R_GridInsertColumn(grid2, l + 1, row, grid1->verts[k + 1 + offset1].xyz, grid1->widthLodError[k + 1]);
			...
			for (m = 0; m < 2; m++) {
				...
				for (l = 0; l < grid2->height - 1; l++) {
					...
					R_GridInsertRow(grid2, l + 1, column, grid1->verts[k + 1 + offset1].xyz, grid1->widthLodError[k + 1]);
		...
		for (k = 0; k < grid1->height - 2; k += 2) {
			for (m = 0; m < 2; m++) {
				...
				for (l = 0; l < grid2->width - 1; l++) {
					...
					R_GridInsertColumn(grid2, l + 1, row, grid1->verts[grid1->width * (k + 1) + offset1].xyz, grid1->heightLodError[k + 1]);
			...
			for (m = 0; m < 2; m++) {
				...
				for (l = 0; l < grid2->height - 1; l++) {
					...
					R_GridInsertRow(grid2, l + 1, column, grid1->verts[grid1->width * (k + 1) + offset1].xyz, grid1->heightLodError[k + 1]);
...
This function will try to stitch patches in the same LoD group together for the highest LoD. Only single missing vertice cracks will be
fixed. Vertices will be joined at the patch side a crack is first found, at the other side of the patch (on the same row or column) the
vertices will not be joined and cracks might still appear at that side.
=======================================================================================================================================
*/
...
static void R_LoadSurfaces(lump_t *surfs, lump_t *verts, lump_t *indexLump) {
	...
	// this ensures surfaces are close together to reduce L2 cache misses when using VAOs, which don't actually use the verts and indexes
...
void RE_LoadWorldMap(const char *name) {
	...
	// clear tr.world so if the level fails to load, the next try will not look at the partially loaded version
	tr.world = NULL;
	...
	if (0) {
		...
		for (i = 0; i < w->numWorldSurfaces; i++) {
			...
			if (ci->type & CULLINFO_BOX) {
				...
				//ri.Printf(PRINT_ALL, "surf %d bounds (%f %f %f)-(%f %f %f) ibounds (%d %d %d)-(%d %d %d)\n", i, ci->bounds[0][0], ci->bounds[0][1], ci->bounds[0][2], ci->bounds[1][0], ci->bounds[1][1], ci->bounds[1][2], ibounds[0][0], ibounds[0][1], ibounds[0][2], ibounds[1][0], ibounds[1][1], ibounds[1][2]);
