...
=======================================================================================================================================
*/
static void ResampleTexture(byte *in, int inwidth, int inheight, byte *out, int outwidth, int outheight) {
	int i, j;
	byte *inrow, *inrow2;
	int frac, fracstep;
	int p1[2048], p2[2048];
...
static void RGBAtoNormal(const byte *in, byte *out, int width, int height, qboolean clampToEdge) {
	...
	// now run sobel filter over height values to generate X and Y then normalize
	for (y = 0; y < height; y++) {
		byte *outbyte = out + y * width * 4;
...
Expects data to be a grid with even (0, 0), (2, 0), (0, 2), (2, 2) etc. pixels filled. Only performs FCBI on specified component.
=======================================================================================================================================
*/
...
Similar to DoFCBIQuick, but just takes the average instead of checking derivatives as well, this operates on all four components.
=======================================================================================================================================
*/
...
Operates in place, quartering the size of the texture. Colors are gamma correct.
=======================================================================================================================================
*/
static void R_MipMapsRGB(byte *in, int inWidth, int inHeight) {
	...
	for (y = inHeight; y; y--, in += stride, in2 += stride) {
		for (x = inWidth; x; x--) {
			for (c = 3; c; c--, in++, in2++) {
				total = downmipSrgbLookup[*(in)] + downmipSrgbLookup[*(in + 4)] + downmipSrgbLookup[*(in2)] + downmipSrgbLookup[*(in2 + 4)];
				*out++ = (byte)(powf(total, 1.0f / 2.2f) * 255.0f);
			}
...
static qboolean RawImage_ScaleToPower2(byte **data, int *inout_width, int *inout_height, imgType_t type, imgFlags_t flags, byte **resampledBuffer) {
	...
	if (picmip && data && resampledBuffer && r_imageUpsample->integer && scaled_width < r_imageUpsampleMaxSize->integer && scaled_height < r_imageUpsampleMaxSize->integer) {
		...
		while (finalwidth > r_imageUpsampleMaxSize->integer || finalheight > r_imageUpsampleMaxSize->integer) {
			finalwidth >>= 1;
			finalheight >>= 1;
		}

		while (finalwidth > glConfig.maxTextureSize || finalheight > glConfig.maxTextureSize) {
			finalwidth >>= 1;
			finalheight >>= 1;
		}
...
/*
=======================================================================================================================================
PixelDataFormatFromInternalFormat
=======================================================================================================================================
*/
...
image_t *R_CreateImage2(const char *name, byte *pic, int width, int height, GLenum picFormat, int numMips, imgType_t type, imgFlags_t flags, int internalFormat) {
	...
	if (glConfig.textureFilterAnisotropic && !cubemap) {
		qglTextureParameteriEXT(image->texnum, textureTarget, GL_TEXTURE_MAX_ANISOTROPY_EXT, mipmap ? (GLint)Com_Clamp(1, glConfig.maxAnisotropy, r_ext_max_anisotropy->integer) : 1);
	}
	...
	switch (internalFormat) {
		...
		case GL_DEPTH_COMPONENT24_ARB:
		case GL_DEPTH_COMPONENT32_ARB:
			// fix for sampling depth buffer on old nVidia cards from http://www.idevgames.com/forums/thread-4141-post-34844.html#pid34844
...
image_t *R_FindImageFile(const char *name, imgType_t type, imgFlags_t flags) {
	...
	if (r_normalMapping->integer && (picFormat == GL_RGBA8) && (type == IMGTYPE_COLORALPHA) && ((flags & checkFlagsTrue) == checkFlagsTrue) && !(flags & checkFlagsFalse)) {
			...
			// blur original image's luma to work with the normal map
			{
				...
				for (y = 1; y < height - 1; y++) {
					...
					for (x = 1; x < width - 1; x++) {
						...
						result = *(picbyte - (width + 1) * 4) + *(picbyte - width * 4) + *(picbyte - (width - 1) * 4) + *(picbyte - 1 * 4) + *(picbyte) + *(picbyte + 1 * 4) + *(picbyte + (width - 1) * 4) + *(picbyte + width * 4) + *(picbyte + (width + 1) * 4);
						result /= 9;
						*blurbyte = result;
						picbyte += 4;
						blurbyte += 1;
