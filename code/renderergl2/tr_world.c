...
Tries to cull surfaces before they are lighted or added to the sorting list.
=======================================================================================================================================
*/
static qboolean R_CullSurface(msurface_t *surf) {
	...
	if (surf->cullinfo.type & CULLINFO_PLANE) {
		...
		*/
		...
		// through the BSP, ICD, and hardware that may cause pixel gaps if an epsilon isn't allowed here
		if (ct == CT_FRONT_SIDED) {
...
The given surface is going to be drawn, and it touches a leaf that is touched by one or more dlights, so try to throw out more dlights
if possible.
=======================================================================================================================================
*/
...
static void R_RecursiveWorldNode(mnode_t *node, uint32_t planeBits, uint32_t dlightBits, uint32_t pshadowBits) {
	...
	do {
		...
		// if the node wasn't marked as potentially visible, exit pvs is skipped for depth shadows
		if (!(tr.viewParms.flags & VPF_DEPTHSHADOW) && node->visCounts[tr.visIndex] != tr.visCounts[tr.visIndex]) {
			return;
		}
		// if the bounding volume is outside the frustum, nothing inside can be visible OPTIMIZE: don't do this all the way to leafs?
		if (!r_nocull->integer) {
			int r;
...
static void R_MarkLeaves(void) {
	...
	// if the cluster is the same and the area visibility matrix hasn't changed, we don't need to mark everything again
...
void R_AddWorldSurfaces(void) {
	...
	// now add all the potentially visible surfaces, also mask invisible dlights for next frame
	{
		int i;
