...
static qboolean ParseStage(shaderStage_t *stage, char **text) {
	...
	while (1) {
		...
		// normalScale <xy> or normalScale <x> <y> or normalScale <x> <y> <height>
			...
			stage->normalScale[3] = atof(token);
		// specularScale <rgb> <gloss> or specularScale <metallic> <smoothness> with r_pbr 1 or specularScale <r> <g> <b> or specularScale <r> <g> <b> <gloss>
...
// 	{"flesh",			0,	SURF_FLESH,			0},							// Tobias: replaced by MAT_*
	...
	{"nodlight",		0,	SURF_NODLIGHT,		0}							// don't ever add dynamic lights // Tobias: remved the ,
...
static qboolean ParseShader(char **text) {
	...
	while (1) {
		...
		// polygonOffset
			...
			continue;
		// entityMergable, allowing sprite surfaces from multiple entities to be merged into one batch. This is a savings for smoke puffs and blood,
		// but can't be used for anything where the shader calcs (not the surface function) reference the entity color or scroll
...
static int CollapseStagesToGLSL(void) {
	...
	if (!skip) {
		// if 2+ stages and first stage is lightmap, switch them, this makes it easier for the later bits to process
...
Arnout: this is a nasty issue. Shaders can be registered after drawsurfaces are generated but before the frame is rendered. This will,
for the duration of one frame, cause drawsurfaces to be rendered with bad shaders. To fix this, need to go through all render commands
and fix sortedIndex.
=======================================================================================================================================
*/
...
static shader_t *FinishShader(void) {
	...
	for (stage = 0; stage < MAX_SHADER_STAGES;) {
		...
		// not a true lightmap but we want to leave existing behaviour in place and not print out a warning
...
Return NULL if not found. If found, it will return a valid shader.
=======================================================================================================================================
*/
...
qhandle_t RE_RegisterShaderFromImage(const char *name, int lightmapIndex, image_t *image, qboolean mipRawImage) {
	...
	for (sh = hashTable[hash]; sh; sh = sh->next) {
		...
		// have to check all default shaders otherwise for every call to R_FindShader
		// with that same strippedName a new default shader is created.
		if ((sh->lightmapIndex == lightmapIndex || sh->defaultShader) && !Q_stricmp(sh->name, name)) { // index by name
			// match found
			return sh->index;
		}
	}
...
When a handle is passed in by another module, this range checks it and returns a valid (possibly default) shader_t to be used
internally.
=======================================================================================================================================
*/
...
static void CreateExternalShaders(void) {
	...
	// HACK to make fogging work correctly on flares. Fog colors are calculated in tr_flare.c already.
