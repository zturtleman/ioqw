...
// a trRefEntity_t has all the information passed in by the client game, as well as some locally derived info
typedef struct {
	refEntity_t e;
...
// shaderProgram_t represents a pair of one GLSL vertex and one GLSL fragment shader
...
typedef struct srfBspSurface_s {
	...
	// lod information, which may be different than the culling information to allow for groups of curves that LOD as a unit
	vec3_t lodOrigin;
	float lodRadius;
	int lodFixed;
	int lodStitched;
...
/*
=======================================================================================================================================
The drawsurf sort data is packed into a single 32 bit value so it can be compared quickly during the qsorting process.
...
// we can't change glConfig_t without breaking DLL/vms compatibility, so store extensions we have here.
typedef struct {
...
void R_SubdividePatchToGrid(srfBspSurface_t *grid, int width, int height, srfVert_t points[MAX_PATCH_SIZE * MAX_PATCH_SIZE]);
...
void RE_StretchPicGradient(float x, float y, float w, float h, float s1, float t1, float s2, float t2, qhandle_t hShader, const float *gradientColor);
void RE_BeginFrame(stereoFrame_t stereoFrame);
