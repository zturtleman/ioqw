...
static qboolean IQM_CheckRange(iqmHeader_t *header, int offset, int count, int size) {
	// return true if the range specified by offset, count and size doesn't fit into the file
...
/*
=======================================================================================================================================
Matrix34Multiply_OnlySetOrigin
=======================================================================================================================================
*/
		...
		switch (vertexarray->type) {
			...
			case IQM_BLENDINDEXES:
				if (blendIndexesType == IQM_INT) {
					...
					Com_Memcpy(iqmData->blendIndexes, (byte *)header + vertexarray->offset, n * sizeof(byte));
				}

				break;
			case IQM_BLENDWEIGHTS:
				if (blendWeightsType == IQM_FLOAT) {
					Com_Memcpy(iqmData->blendWeights.f, (byte *)header + vertexarray->offset, n * sizeof(float));
				} else {
					Com_Memcpy(iqmData->blendWeights.b, (byte *)header + vertexarray->offset, n * sizeof(byte));
				}
...
void R_AddIQMSurfaces(trRefEntity_t *ent) {
	...
	// this will write directly into the entity structure, so when the surfaces are rendered, they don't need to be
	// range checked again.
...
static void ComputePoseMats(iqmData_t *data, int frame, int oldframe, float backlerp, float *mat) {
	...
	if (data->num_poses == 0) {
		for (i = 0; i < data->num_joints; i++, joint++) {
			if (*joint >= 0) {
				Matrix34Multiply(mat + 12 * *joint, identityMatrix, mat + 12 * i);
			} else {
	...
	if (oldframe == frame) {
		...
		for (i = 0; i < data->num_poses; i++, joint++) {
			if (*joint >= 0) {
				...
				InterpolateMatrix(mat1 + 12 * i, mat2 + 12 * i, backlerp, mat);
			}
		}
	}
}
...
void RB_IQMSurfaceAnim(surfaceType_t *surface) {
	...
	for (i = 0; i < surf->num_vertexes; i++, outXYZ++, outNormal += 4, outTexCoord++, outColor += 4) {
		int j, k;
		float vtxMat[12];
		float nrmMat[9];
		...
		if (data->num_poses == 0 || numWeights == 0) {
			...
			// compute the vertex matrix by blending the up to four blend weights
		...
		// compute the normal matrix as transpose of the adjoint of the vertex matrix
		...
		(*outXYZ)[0] = vtxMat[0] * data->positions[3 * vtx + 0] + vtxMat[1] * data->positions[3 * vtx + 1] + vtxMat[2] * data->positions[3 * vtx + 2] + vtxMat[3];
		(*outXYZ)[1] = vtxMat[4] * data->positions[3 * vtx + 0] + vtxMat[5] * data->positions[3 * vtx + 1] + vtxMat[6] * data->positions[3 * vtx + 2] + vtxMat[7];
		(*outXYZ)[2] = vtxMat[8] * data->positions[3 * vtx + 0] + vtxMat[9] * data->positions[3 * vtx + 1] + vtxMat[10] * data->positions[3 * vtx + 2] + vtxMat[11];
		(*outXYZ)[3] = 1.0f;
