...
Generates an orientation for an entity and viewParms. Does NOT produce any GL calls. Called by both the front end and the back end.
=======================================================================================================================================
*/
void R_RotateForEntity(const trRefEntity_t *ent, const viewParms_t *viewParms, orientationr_t *or) {
	...
	// calculate the viewer origin in the model's space needed for fog, specular, and environment mapping
...
Set up the culling frustum planes for the current view using the results we got from computing the first two rows of the projection
matrix.
=======================================================================================================================================
*/
...
void R_SetupProjection(viewParms_t *dest, float zProj, float zFar, qboolean computeFrustum) {
	...
	// offset the view origin of the viewer for stereo rendering by setting the projection matrix appropriately
	if (stereoSep != 0) {
...
void R_AddDrawSurf(surfaceType_t *surface, shader_t *shader, int fogIndex, int dlightMap, int pshadowMap, int cubemap) {
	...
	// the sort data is packed into a single 32 bit value so it can be compared quickly during the qsorting process
...
void R_SortDrawSurfs(drawSurf_t *drawSurfs, int numDrawSurfs) {
	...
	// check for any pass through drawing, which may cause another view to be rendered first
...
static void R_AddEntitySurface(int entityNum) {
	...
	// we don't want the hacked weapon position showing in mirrors, because the true body position will already be drawn
...
void R_GenerateDrawSurfs(void) {
	...
	// this needs to be done before entities are added, because they use the projection
...
void R_RenderView(viewParms_t *parms) {
	...
	R_GenerateDrawSurfs();
	// if we overflowed MAX_DRAWSURFS, the drawsurfs wrapped around in the buffer and we will be missing
