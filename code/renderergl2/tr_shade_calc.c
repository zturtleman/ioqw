...
static void Autosprite2Deform(void) {
	...
	// we could precalculate a lot of it is an issue, but it would mess up the shader abstraction
...
void RB_CalcModulateColorsByFog(unsigned char *colors) {
	...
	// calculate texcoords so we can derive density this is not wasted, because it would only have been previously called if the surface
	// was opaque
...
To do the clipped fog plane really correctly, we should use projected textures, but I don't trust the drivers and it doesn't fit our
shader data.
=======================================================================================================================================
*/
void RB_CalcFogTexCoords(float *st) {
	...
	// see if the viewpoint is outside this is needed for clipping distance even for constant fog
	if (eyeT < 0) {
		eyeOutside = qtrue;
	} else {
		eyeOutside = qfalse;
...
void RB_CalcRotateTexMatrix(float degsPerSecond, float *matrix) {
	...
	matrix[0] = cosValue;
	matrix[2] = -sinValue;
	matrix[4] = 0.5 - 0.5 * cosValue + 0.5 * sinValue;
	matrix[1] = sinValue;
	matrix[3] = cosValue;
	matrix[5] = 0.5 - 0.5 * sinValue - 0.5 * cosValue;
}
