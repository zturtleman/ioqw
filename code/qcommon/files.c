...
When building a pak file, make sure a qwconfig.cfg isn't present in it,
			...
			do {
				...
				if (!FS_FilenameCompare(pakFile->name, filename)) {
					...
					if (!(pak->referenced & FS_GENERAL_REF)) {
						if (!FS_IsExt(filename, ".shader", len) && !FS_IsExt(filename, ".txt", len) && !FS_IsExt(filename, ".cfg", len) && !FS_IsExt(filename, ".config", len) && !FS_IsExt(filename, ".bot", len) && !FS_IsExt(filename, ".arena", len) && !FS_IsExt(filename, ".menu", len) && Q_stricmp(filename, "vm/qagame.qvm") != 0 && !strstr(filename, "levelshots")) {
...
A mod directory is a peer to base game with a pk3 in it. The directories are searched in base path, cd path and home path.
=======================================================================================================================================
*/
...
qboolean FS_ConditionalRestart(int checksumFeed, qboolean disconnect) {
	...
	if (fs_gamedirvar->modified) {
		if (FS_FilenameCompare(lastValidGame, fs_gamedirvar->string) && (*lastValidGame || FS_FilenameCompare(fs_gamedirvar->string, com_basegame->string)) && (*fs_gamedirvar->string || FS_FilenameCompare(lastValidGame, com_basegame->string))) {
