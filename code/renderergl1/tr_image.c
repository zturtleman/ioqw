...
image_t	*R_FindImageFile( const char *name, imgType_t type, imgFlags_t flags )
{
	...
	if ( ( flags & IMGFLAG_LIGHTMAP ) && picFormat == GL_RGBA8 ) {
		R_ProcessLightmap( &pic, 4, width, height, &pic, qfalse, picFormat );
	}
	else if (r_normalMapping->integer && (picFormat == GL_RGBA8) && (type == IMGTYPE_COLORALPHA) &&
		(flags & IMGFLAG_PICMIP) && (flags & IMGFLAG_MIPMAP) && (flags & IMGFLAG_GENNORMALMAP) && !(flags & IMGFLAG_CUBEMAP))
	{
	...
	image = R_CreateImage2( ( char * ) name, pic, width, height, picFormat, picNumMips, type, flags, textureInternalFormat );
	ri.Free( pic );
	return image;
}
