...
void S_Base_BeginRegistration(void) {
	...
	if (s_numSfx == 0) {
		...
		S_Base_RegisterSound("snd/u/hit.wav", qfalse);
	}
}
...
void S_SpatializeOrigin(vec3_t origin, int master_vol, int *left_vol, int *right_vol, int range, int volume) {
	...
	dist -= dist_fullvol;
	// if close enough to be at full volume
	if (dist < 0.0f) {
		dist = 0.0f;
	} else {
		dist /= range;
...
static void S_OpenBackgroundStream(const char *filename) {
	...
	if (s_backgroundStream->info.channels != 2 || s_backgroundStream->info.rate != 48000) {
		Com_Printf(S_COLOR_YELLOW "WARNING: music file %s is not 48kHz stereo\n", filename);
	}
}
...
qboolean S_Base_Init(soundInterface_t *si) {
	...
	s_mixahead = Cvar_Get("s_mixahead", "0.25", CVAR_ARCHIVE);
	s_mixPreStep = Cvar_Get("s_mixPreStep", "0.1", CVAR_ARCHIVE);
