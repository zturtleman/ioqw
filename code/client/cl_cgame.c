...
intptr_t CL_CgameSystemCalls(intptr_t *args) {
	...
	switch (args[0]) {
		...
		case CG_FS_READ:
#ifdef NEW_FILESYSTEM
			if (fs_handle_get_owner(args[3]) != FS_HANDLEOWNER_CGAME) {
				return 0;
			}
#endif
		...
		case CG_FS_SEEK:
#ifdef NEW_FILESYSTEM
			if (fs_handle_get_owner(args[1]) != FS_HANDLEOWNER_CGAME) {
				return 0;
			}
#endif
...
#ifdef NEW_FILESYSTEM
	// give the map pk3 slightly higher precedence in the filesystem to help load the correct textures and such
...
void CL_FirstSnapshot(void) {
	...
	if (!clc.voipCodecInitialized) {
		...
		Cvar_Set("cl_voipSendTarget", "team");
