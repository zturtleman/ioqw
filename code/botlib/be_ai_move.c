...
//movement state
//NOTE: the moveflags MFL_ONGROUND, MFL_TELEPORTED and MFL_WATERJUMP
//		must be set outside the movement code
...
void BotInitMoveState(int handle, bot_initmove_t *initmove)
{
	...
	if (initmove->or_moveflags & MFL_SCOUT) ms->moveflags |= MFL_SCOUT;
...
int BotOnMover(vec3_t origin, int entnum, aas_reachability_t *reach)
{
	...
	//
	trace = AAS_Trace(org, boxmins, boxmaxs, end, entnum, CONTENTS_SOLID|CONTENTS_PLAYERCLIP|CONTENTS_BOTCLIP);
	...
	if (ms->moveflags & MFL_ONGROUND)
	{
		...
		if (type & MOVE_JUMP)
		{
			...
			stopevent = SE_HITGROUND|SE_HITGROUNDDAMAGE|
						SE_ENTERWATER|SE_ENTERSLIME|SE_ENTERLAVA|SE_GAP;
		} //end if
		else
		{
			maxframes = 2;
		...
		if (move.stopevent & (SE_ENTERSLIME|SE_ENTERLAVA|SE_HITGROUNDDAMAGE|SE_GAP))
		{
...
bot_moveresult_t BotTravel_WalkOffLedge(bot_movestate_t *ms, aas_reachability_t *reach)
{
	...
	if (dist < 64)
	{
		...
		AAS_PredictClientMovement(&move, ms->entitynum, reach->end, PRESENCE_NORMAL, qtrue, velocity, cmdmove, 2, 2, 0.1f, SE_HITGROUNDDAMAGE|SE_ENTERSLIME|SE_ENTERLAVA|SE_TOUCHJUMPPAD, 0, qfalse);
		// check for nearby gap
...
bot_moveresult_t BotTravel_Elevator(bot_movestate_t *ms, aas_reachability_t *reach)
{
	...
	else
	{
		...
		if (dist < 64)
		{
			...
			{
				if (dist > 50) EA_Move(ms->client, dir, speed);
			} //end if
			VectorCopy(dir, result.movedir);
			//
			if (ms->moveflags & MFL_SWIMMING) result.flags |= MOVERESULT_SWIMVIEW;
...
bot_moveresult_t BotMoveInGoalArea(bot_movestate_t *ms, bot_goal_t *goal)
{
	...
	dist = VectorNormalize(dir);
	if (dist > 100) dist = 100;
	if (ms->moveflags & MFL_WALK) speed = 200 - (200 - 2 * dist);
	else speed = 400 - (400 - 4 * dist);
	if (speed < 10) speed = 0;
	//
	BotCheckBlocked(ms, dir, qtrue, &result);
	//elementary action move in direction
