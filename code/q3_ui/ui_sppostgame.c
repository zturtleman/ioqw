...
char *ui_medalSounds[] = {
	"snd/v/voc_accuracy.wav",
	"snd/v/voc_impressive_a.wav",
	"snd/v/voc_excellent_a.wav",
	"snd/v/voc_gauntlet.wav",
	"snd/v/voc_frags.wav",
	"snd/v/voc_perfect.wav"
};
...
void UI_SPPostgameMenu_Cache(void) {
	...
	if (buildscript) {
		...
		trap_S_RegisterSound("snd/v/youwin.wav", qfalse);
	}
}
...
void UI_SPPostgameMenu_f(void) {
	...
	for (n = 0; n < postgameMenuInfo.numClients; n++) {
		...
		postgameMenuInfo.scores[n] = atoi(UI_Argv(8 + n * 3 + 2));
		postgameMenuInfo.ranks[n] = atoi(UI_Argv(8 + n * 3 + 3));
	...
	awardValues[AWARD_FRAGS] = atoi(UI_Argv(3));
	awardValues[AWARD_ACCURACY] = atoi(UI_Argv(4));
	awardValues[AWARD_EXCELLENT] = atoi(UI_Argv(5));
	awardValues[AWARD_IMPRESSIVE] = atoi(UI_Argv(6));
	awardValues[AWARD_GAUNTLET] = atoi(UI_Argv(7));
	...
	if (playerGameRank != 1) {
		postgameMenuInfo.winnerSound = trap_S_RegisterSound(va("snd/v/%s_wins.wav", postgameMenuInfo.placeNames[0]), qfalse);
		trap_Cmd_ExecuteText(EXEC_APPEND, "music music/loss\n");
	} else {
		postgameMenuInfo.winnerSound = trap_S_RegisterSound("snd/v/youwin.wav", qfalse);
