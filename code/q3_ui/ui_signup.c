...
static void Signup_MenuEvent(void *ptr, int event) {
	...
	switch (((menucommon_s *)ptr)->id) {
		case ID_SIGNUP:
			if (strcmp(s_signup.password_box.field.buffer, s_signup.again_box.field.buffer) != 0) {
			...
			*/
			// create account
			/*
			sprintf(cmd, "cmd rank_create \"%s\" \"%s\" \"%s\"\n", s_signup.name_box.field.buffer, s_signup.password_box.field.buffer, s_signup.email_box.field.buffer);
