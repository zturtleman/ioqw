...
float BotAggression(bot_state_t *bs) {
	...
	if (bs->inventory[INVENTORY_BFG10K] > 0 &&
			bs->inventory[INVENTORY_BFG_AMMO] > 7) return 100;
	...
	if (bs->inventory[INVENTORY_BEAMGUN] > 0 &&
			bs->inventory[INVENTORY_BEAMGUN_AMMO] > 50) return 90;
...
int BotHasPersistantPowerupAndWeapon(bot_state_t *bs) {
	...
	//if the bot can use the bfg
	if (bs->inventory[INVENTORY_BFG10K] > 0 &&
			bs->inventory[INVENTORY_BFG_AMMO] > 7) return qtrue;
	//if the bot can use the railgun
	if (bs->inventory[INVENTORY_RAILGUN] > 0 &&
			bs->inventory[INVENTORY_SLUGS] > 5) return qtrue;
	//if the bot can use the beam gun
	if (bs->inventory[INVENTORY_BEAMGUN] > 0 &&
			bs->inventory[INVENTORY_BEAMGUN_AMMO] > 50) return qtrue;
	//if the bot can use the rocketlauncher
	if (bs->inventory[INVENTORY_ROCKETLAUNCHER] > 0 &&
...
float BotEntityVisible(int viewer, vec3_t eye, vec3_t viewangles, float fov, int ent) {
	...
	if (EntityIsInvisible(&entinfo) && VectorLength(dir) > 300) return 0;
...
void BotAimAtEnemy(bot_state_t *bs) {
	...
	if (enemyvisible) {
		...
		if (aim_skill > 0.5) {
			...
			if (wi.number == WP_BFG ||
				wi.number == WP_ROCKETLAUNCHER ||
				wi.number == WP_GRENADELAUNCHER) {
				//create the chase goal
...
void BotCheckForProxMines(bot_state_t *bs, entityState_t *state) {
	...
	if (!(bs->inventory[INVENTORY_PLASMAGUN] > 0 && bs->inventory[INVENTORY_CELLS] > 0) &&
		...
		!(bs->inventory[INVENTORY_BFG10K] > 0 && bs->inventory[INVENTORY_BFG_AMMO] > 0) ) {
		return;
	}
