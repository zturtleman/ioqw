...
cd `dirname $0`
if [ ! -f Makefile ]; then
	echo "This script must be run from the quakewars build directory"
	exit 1
fi
...
# enough gcc to actually compile Quake Wars
# For PPC macs, G4's or better are required to run Quake Wars.
